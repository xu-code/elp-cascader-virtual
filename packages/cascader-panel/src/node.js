...
export default class Node {
  ...
  constructor (data, config, parentNode) {
    ...
    this.initState()
    this.initChildren()
    this.onChildCheck = debounce(60, this.debounceOnChildCheck)
  }
  ...
  // 利用hash表缓存数据，减少计算量（空间换时间）
  getCheckedValueMap (checkedValue, value) {
    if (!checkedValue) return false
    if ($cacheCheckedValue !== checkedValue) {
      $cacheCheckedValue = checkedValue
      $checkedValueMap = {}
      checkedValue.forEach(item => {
        $checkedValueMap[item.join()] = true
      })
    }
    return $checkedValueMap[value.join()] || false
  }
  isSameNode (checkedValue) {
    const value = this.getValueByOption()
    return this.config.multiple && Array.isArray(checkedValue)
      ? this.getCheckedValueMap(checkedValue, value)
  ...
  setCheckState (checked) {
    ...
    Vue.prototype.$set(this, 'checked', checked)
    Vue.prototype.$set(this, 'indeterminate', checkedNum !== totalNum && checkedNum > 0)
  }
